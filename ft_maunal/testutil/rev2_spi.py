...
def selftest():
    ...
    rev2 = Rev2Spi(listPorts=True)

    rev2 = Rev2Spi("COM8")
    print(rev2.get_current())
...
class Rev2Spi:
    ...
    def writeLUT0(self, i_n, i_p, q_n, q_p):
        ...
        self.write("wfe00{:02x}".format(self.reg0))
        self.write("wfe01{:02x}".format(self.reg1))
        self.write("wfe80{:02x}".format(self.reg128))

    def printLUT0(self):
        print(self.reg0, self.reg1, self.reg128)
        print("w00{:02x} {:02x} {:02x}".format(self.reg0, self.reg1, self.reg128))

    # set PSR[1-4] to LUT0, load psr by hardware load
    def _loadPSR_hw(self):
        self.write("wa000")
        self.write("wa100")
        self.write("wa200")
        self.hw_npsrld()

    # set PSR[1-4] to LUT0, load psr by write something to reg163
    def _loadPSR_sw(self):
        self.write("wa000")
        self.write("wa100")
        self.write("wa200")
        self.write("wa3a3")

    def set_iout(self, channel_offset, value):
        regIndex_offset = 183
        reg_no = regIndex_offset + channel_offset
        string = "wf8{:02x}{:02x}".format(reg_no, value)
        self.write(string)
    ...
    def set_channel_psr(self, channel, value):
        ...
        regIndex_offset = 0;
        all_channel_flag = 0
        data = value + (channel << 11) + (all_channel_flag << 15);
        string = "wfb{:04x}".format(data)
        self.write(string)
        self.write(string) # rev2 spi bug: must write twice
        self.hw_npsrld()
    ...
    def set_channel_atn(self, channel, value):
        ...
        regIndex_offset = 0;
        all_channel_flag = 0
        data = (value << 6) + (channel << 11) + (all_channel_flag << 15);
        string = "wfa{:04x}".format(data)
        self.write(string)
        self.write(string) # rev2 spi bug: must write twice
        self.hw_npsrld()
