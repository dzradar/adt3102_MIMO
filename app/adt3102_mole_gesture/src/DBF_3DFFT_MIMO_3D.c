  ...
  //arm_fill_f32(0, &data_out[0][0], FFT_3D * N_fft_half * 2);
    ...
    *((int32 *)(target_list+3)) = 0;
    ...
    for (i = 0; i < tar_num; i++)
    {
        ...
        tmp1   = rdata_ch1 * rdata_ch2 + idata_ch1 * idata_ch2;
        tmp2   = rdata_ch1 * idata_ch2 - idata_ch1 * rdata_ch2;
        dphi4 = atan2(tmp2, tmp1);           

        tmp1 = asin(dphi1/3.1416f);  // good for azi
        tmp2 = asin(dphi2/3.1416f);
        tmp3 = asin(dphi3/3.1416f);  // good for ele
        tmp4 = asin(dphi4/3.1416f);
        // mean       
        if (( fabs(tmp1 - tmp2) > 0.2) || ( fabs(tmp3 - tmp4) > 0.2) )
            continue;  

        //theta = tmp3;
        //phi = tmp1;
        theta = asin((dphi3 + dphi4)/2.0f/3.1416f);
        phi = asin((dphi1 + dphi2)/2.0f/3.1416f);

        //if ((fabs(theta) > 40) || (fabs(phi)> 45))
        //    continue;
